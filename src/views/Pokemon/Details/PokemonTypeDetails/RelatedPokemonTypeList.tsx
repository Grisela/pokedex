...
import { IPokemonListTypeChild, IPokemonTypeDetailsQuery } from "./types";
    ...
    <Box pl={5} mt={50}>
      ...
      >
        {isEmpty(state.data?.pokemon) ? (
          ...
          sortBy(
            state.data?.pokemon,
            (pokemon: IPokemonListTypeChild) => pokemon.pokemon.name
          ).map((e: IPokemonListTypeChild, idx: number) => (
            <Grid.Col
